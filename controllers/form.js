...
exports.assignReport = async (req, res) => {
  try {
    ...
    const assignedReport = await Form.findByIdAndUpdate(
      req.params.formId,
      { reportId: req.body.reportId, 
       isReportAssigned: isAssigned },
      { new: true }
    );
    if(!assignedReport) {
      const err = new Error(error_code.UNKNOWN_ERROR.CODE);
...
exports.formsAnalyticalChart = async (req, res) => {
  try {
    const checkValidUser = await Users.findById(req.sessionUserData.userid);
    if (!checkValidUser) {
      const err = new Error(error_code.USER_NOT_FOUND.CODE);
    ...
    const formCount = await form.countDocuments();
    const userCount = await Users.countDocuments({isSuperAdmin: false});
...
exports.popularFormChart = async (req, res) => {
  try {
    const checkValidUser = await Users.findById(req.sessionUserData.userid);
    if (!checkValidUser) {
      const err = new Error(error_code.USER_NOT_FOUND.CODE);
    ...
    let top6Forms = [];
    for (const singleForm of allForm) {
      const form_id = singleForm._id;
      const respCount = await Response.countDocuments({ formId: form_id });
      top6Forms.push({ formId: form_id, responseCount: respCount });
      top6Forms.sort((a, b) => b.responseCount - a.responseCount);
      top6Forms = top6Forms.slice(0, 6);
    }
    if (top6Forms.length === 6) {
      const lastIndex = top6Forms[3].responseCount;
      let popularForms = [];
      for (const singleForm of allForm) {
        const form_id = singleForm._id;
        const formTitle = singleForm.title;
        const respCount = await Response.countDocuments({ formId: form_id });
        if (respCount >= lastIndex) {
          popularForms.push({ formTitle: formTitle, responseCount: respCount });
        }
      }
      return res.status(200).json({ success: true, data: popularForms, });
    }
  } catch (error) {
    console.log(error);
  }
...
exports.checkFreePremiumOrgUser = async (req, res) => {
  try {
    const checkValidUser = await Users.findById(req.sessionUserData.userid);
    if (!checkValidUser) {
      const err = new Error(error_code.USER_NOT_FOUND.CODE);
    ...
    const countOrg = await Organization.countDocuments();
    const countFreeOrg = await Organization.countDocuments({subscriptionType: 'free'});
    const countPremiumOrg = await Organization.countDocuments({subscriptionType: 'premium'});
    const findFreeOrg = await Organization.find({subscriptionType: 'free'});
    var countFreeUser = 0;
    for (const org of findFreeOrg) {
      const userCount = await Users.countDocuments({'permissions.organizationId': org._id});
      countFreeUser += userCount;
    }
    const findPremiumOrg = await Organization.find({subscriptionType: 'premium'});
    var countPremiumUser = 0;
    for (const org of findPremiumOrg) {
      const userCount = await Users.countDocuments({'permissions.organizationId': org._id});
      countPremiumUser += userCount;
    }
    const allResponses = await Response.find();
    const responseUserSet = new Set(); 
    allResponses.forEach(response => {
      if (response.submittedBy.userId) {
        responseUserSet.add(response.submittedBy.userId.toString());
      }
    });
    const usersWithResponsesCount = responseUserSet.size;
    const dataToSend = {
      totalOrg: countOrg, 
      freeOrg: countFreeOrg, 
      premiumOrg: countPremiumOrg, 
      freeOrgUser: countFreeUser, 
      premiumOrgUser: countPremiumUser, 
      usersWithResponses: usersWithResponsesCount
    };
    return res.status(200).json({
      dataToSend
    });
...
exports.countEverything = async (req, res) => {
  try {
    const checkValidUser = await Users.findById(req.sessionUserData.userid);
    if (!checkValidUser) {
      const err = new Error(error_code.USER_NOT_FOUND.CODE);
    ...
    const countOwner = await Users.countDocuments({'permissions.type': 'owner'});
    const countActiveOwner = await Users.countDocuments({'permissions.type': 'owner', status: 'active'});
    const countMember = await Users.countDocuments({'permissions.type': 'member'});
    const countActiveMember = await Users.countDocuments({'permissions.type': 'member', status: 'active'});
    const countForm = await Form.countDocuments();
    const countOrg = await Organization.countDocuments();
    const countResp = await Response.countDocuments();
    const countTeam = await Teams.countDocuments();
    const dataToSend = {
      countOwner: countOwner, 
      countActiveOwner: countActiveOwner, 
...
exports.countCreateFormPerDay = async (req, res) => {
  try {
    const checkValidUser = await Users.findById(req.sessionUserData.userid);
    if (!checkValidUser) {
      const err = new Error(error_code.USER_NOT_FOUND.CODE);
    ...
    const allForm = await Form.find();
    const countFormByDay = {};
    const countFormPerDay = [];
    allForm.forEach(singleForm => {
      var formCreatedAt = new Date(singleForm.createdAt).toISOString().split('T')[0];
      if (!countFormByDay[formCreatedAt]) {
        countFormByDay[formCreatedAt] = { count: 1, forms: [singleForm._id] };
      } else {
        countFormByDay[formCreatedAt].count++;
        countFormByDay[formCreatedAt].forms.push(singleForm._id);
      }
    });
    const sortedForms = Object.entries(countFormByDay)
    .sort((a, b) => b[1].count - a[1].count) 
    .slice(0, 5) 
    .map(([formCreatedAt, data]) => ({
      formCreatedAt,
      countFormPerDay: data.count,
    }));
    const lastPosition = sortedForms[4].countFormPerDay;
    const topFormDays = [];
    for (const [formCreatedAt, formInfo] of Object.entries(countFormByDay)) {
      if (formInfo.count >= lastPosition) {
        topFormDays.push({ formCreatedAt, count: formInfo.count });
      }
    }
    const dataToSend = {
      countFormByDay: topFormDays
    };
    return res.status(200).json({
      dataToSend
    });

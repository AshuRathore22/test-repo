...
class ExcelClass {
  async importMainRes(filePath, mainBody) {
    try {
      ...
      let jsonData = XLSX.utils.sheet_to_json(worksheet["Sheet1"],{raw: false, defval:""});
          ...
          $project: {
            questions: {
              $filter: {
                input: "$questions",
                as: "questions",
                cond: { $eq: ["$$questions.isGroupChild", false] },
              },
            },
          },
        }
      ...
      for (let row of jsonData) {
        ...
        const created_on = row[submittedKey] ? moment(row[submittedKey].replace('/','-'), 'DD-MM-YYYY').format() : undefined;
        const updated_on = row[modifiedKey] ? moment(row[modifiedKey].replace('/','-'), 'DD-MM-YYYY').format() : undefined;
        ...
        const getUserId = await Users.findOne(
          { mobile: { $regex: row["Username"].slice(4, row["Username"].length) } },
          { _id: 1 }
        );
        if (!getUserId) {
        ...
        const time_spent = row['Time Spent (hh:mm:ss)'] ? this.timeToTimestamp(row['Time Spent (hh:mm:ss)']) : undefined;
        const response_note = row['Response Note'] ? row['Response Note'] : undefined;
        const response_tag = row['Tags'] ? row['Tags'].toLowerCase() : 'no_tag';
        const form_revision = [
          {
            createdIn: row['Form Revision Created In'] ? 'v'+row['Form Revision Created In'] : 'v1',
            submittedIn: row['Form Revision Submitted In'] ? 'v'+row['Form Revision Submitted In'] : 'v1',
            lastModifiedIn: row['Form Revision Submitted In'] ? 'v'+row['Form Revision Submitted In'] : 'v1'
          }
        ];
        
        const keyObj = Object.keys(row).slice(Object.keys(row).indexOf("Time Spent (hh:mm:ss)")+1, Object.keys(row).length);

        const responses = []; let idx=0
        for (let formQues of getFormData[0].questions) {
          ...
          if (formQues.questionType === "mcq_single") {
            if (!(row[keyObj[idx]]==="" || row[keyObj[idx]]==="N/A")) {
              if (moment(row[keyObj[idx]]).format() !== 'Invalid date') {
                resValueObj.value = formQues.properties.options.filter(opt=>moment(opt.label).format()==moment(row[keyObj[idx]]).format())[0].id;
              } else {
                const optToMatch = row[keyObj[idx]].replace(/[^a-zA-Z0-9 ]/g, "").trim();
                resValueObj.value = formQues.properties.options.filter(opt=>opt.label.replace(/[^a-zA-Z0-9 ]/g, "").match(new RegExp(optToMatch)))[0].id;
              }
            }
            idx++;

          } else if (formQues.questionType === "mcq_multiple") {
            let title = keyObj.filter(val=>val.split("_")[0]===formQues.title);
            if (!title.length) {
              title = keyObj.filter(val=>{
                let h = val.split("_");h.pop();
            ...
            if (!(row[keyObj[idx]]==="" || row[keyObj[idx]]==="N/A")) {
              ...
              for (let i=0; i<title.length; i++) {
                if (row[title[i]]==="1" || row[title[i]]==='Yes') {
                  ...
                  const optToMatch = title[i].split("_")[title[i].split("_").length-1].replace(/[^a-zA-Z0-9 ]/g, "").trim();
                  // console.log(optToMatch);
                  mcqMultiRes.push(formQues.properties.options.filter(val=>val.label.replace(/[^a-zA-Z0-9 ]/g, "").match(new RegExp(optToMatch)))[0].id);
                }
              }
            ...
            if (!(row[keyObj[idx]]==="" || row[keyObj[idx]]==="N/A")) {
              const title = keyObj.filter(val=>val.split("_")[0]===formQues.title);
              if (!title.length) {
                title = keyObj.filter(val=>{
                  let h = val.split("_");h.pop();
                  return h.join("_")===formQues.keyword
              ...
              if (title.length) {
                ...
                locRes.longitude = parseFloat(row[`${formQues.title}_longitude`]);
            ...
            if (!(row[keyObj[idx]]==="" || row[keyObj[idx]]==="N/A")) {
              const mediaData = await downloadFileHelper.downloadFile(row[keyObj[idx]], Date.now()+"."+row[keyObj[idx]].split(".")[row[keyObj[idx]].split(".").length-1]);
              const mediaResValue = {
                "url": mediaData,
                "mimeType": `image/${mediaData.split(".")[mediaData.split(".").length-1]}`
              }
          ...
          // -----------dropdown--
            ...
            if (!(row[keyObj[idx]]==="" || row[keyObj[idx]]==="N/A")) {
              ...
              const mediaResValue = {
                "url": mediaData,
                "mimeType": `audio/${mediaData.split(".")[mediaData.split(".").length-1]}`
              }
              resValueObj.value = mediaResValue;
            }
            idx++;
            ...
            if (!(row[keyObj[idx]]==="" || row[keyObj[idx]]==="N/A")) {
              ...
              const mediaResValue = {
                "url": mediaData,
                "mimeType": `video/${mediaData.split(".")[mediaData.split(".").length-1]}`
              }
              resValueObj.value = mediaResValue;
            }
            idx++;
            ...
            if (!(row[keyObj[idx]]==="" || row[keyObj[idx]]==="N/A")) {
              ...
              const mediaResValue = {
                "url": mediaData,
                "mimeType": `file/${mediaData.split(".")[mediaData.split(".").length-1]}`
              }
              resValueObj.value = mediaResValue;
            }
            idx++;
          ...
          // else if (formQues.questionType === "grp_no_repeat") {
          //   idx++;
            
          // } else if (formQues.questionType === "grp_number") {
          //   idx++;

          // } else if (formQues.questionType === "grp_choice") {
          //   idx++;

          // } else if (formQues.questionType === "grp_custom") {
          //   idx++;
        ...
        const upsertAllResponse = await Response.updateOne({importedId: row["Response ID"]}, putResponse, {upsert: true});
        ...
        // console.log(upsertAllResponse);
      }

      // const updatedData = [];
      // allImportingData.forEach(async (val) => {
      //   const upsertAllResponse = await Response.updateOne({importedId: val.importedId}, val, {upsert: true});
      //   if (!upsertAllResponse) {
      //     const err = new Error(error_code.UNKNOWN_ERROR.CODE);
      //     err.statusCode = 0;
      //     throw err;
      //   }
      //   console.log(upsertAllResponse);
      //   updatedData.push(upsertAllResponse);
      // });

      // const insertAllResponse = await Response.insertMany(allImportingData);
      // if (!insertAllResponse) {
      //   const err = new Error(error_code.UNKNOWN_ERROR.CODE);
      //   err.statusCode = 0;
      //   throw err;
      // }
      ...
      return Promise.resolve({
        ...
        // data: jsonData,
        inserted: allImportingData.filter(val=>val?.upsertedCount===1).length,
        // ignored: countIgnored,
        updated: allImportingData.filter(val=>val?.modifiedCount===1).length,
        // data: allImportingData,
        message: "Uploaded",
        status: 1
      });
    } catch (error) {
  ...
  async importForm(file, mainBody, sessionUserData) {
    try {
      ...
      let jsonData = await Papa.parsePromise(dataStream);
      

      var groupedArr = jsonData.data.reduce(function(results, data) {
      ...
      for (let props in groupedArr){
        ...
        for (let i = 0; i < jsonData.length; i++ ) {
          ...
          const description = jsonData[i]['Description']?jsonData[i]['Description']:null;
          const keyword = jsonData[i]['Keyword']?jsonData[i]['Keyword']:null;
          const mandatory = jsonData[i]['Mandatory'] === 'true' || jsonData[i]['Mandatory'] === 'TRUE';
          ...
          const markOnMap = jsonData[i]['Mark On Map'] === 'true' || jsonData[i]['Mark On Map'] === 'TRUE';
          const galleryUploadAllowed = jsonData[i]['Gallery Upload allowed'] === 'true' || jsonData[i]['Gallery Upload allowed'] === 'TRUE';
          const allowCaption = jsonData[i]['Allow Caption'] === 'true' || jsonData[i]['Allow Caption'] === 'TRUE';
          const allowDecimalNumber = jsonData[i]['Allow Decimal Number'] === 'true' || jsonData[i]['Allow Decimal Number'] === 'TRUE';
          ...
          const groupedQuestionChild = jsonData[i]['Grouped Question Child'] === 'true' || jsonData[i]['Grouped Question Child'] === 'TRUE';
          const groupType = jsonData[i]['Group Type'];
          const groupLabels = jsonData[i]['Group Labels'];
          const groupLabelsDependentOn = jsonData[i]['Group Labels Dependent On'];
          const option = jsonData[i]['Option'];
          const optionActive = jsonData[i]['Option Active'] === 'true' || jsonData[i]['Option Active'] === 'TRUE';
          ...
          if(questionType === 'Section Break') {
            ...
            questionObj.properties.measurementUnit = 'Squared meter (m^2)';
            formObj.questions.push(questionObj);
            ...
            questionObj.properties.validationType = 'valueBased';
            questionObj.properties.isDecimalAllowed = allowDecimalNumber;
            questionObj.properties.maxLimit = Number(upperLimit);
            questionObj.properties.minLimit = lowerLimit?Number(lowerLimit):0;
            ...
            questionObj.properties.measurementUnit = 'Meter (m)';
            formObj.questions.push(questionObj);
          }
          // ------------------dropdown
          else if(questionType === 'dropdown on Map') {
          ...
          // ----------------
            ...
            if(defaultCountry){
                 ...
                 let countryCode = countryCodes.find(o => o.country == countryName);
                 if(countryCode && Object.keys(countryCode).length !== 0){
                  questionObj.properties.countryCode = countryCode.calling_code.toString();
                 } else {
            ...
            questionObj.properties.isAudioMuted = false;
            if(imageQuality == 'low'){
          ...
          // -------multiple File----------
            ...
            questionObj.properties.ratingType = 'likert';
            questionObj.properties.options = [{"label":"Extremely Unsatisfied","code":""},{"label":"Unsatisfied","code":""},{"label":"Neutral","code":""},{"label":"Satisfied","code":""},{"label":"Extremely Satisfied","code":""}];
            formObj.questions.push(questionObj);
          } else if(questionType === 'Scale') {
            questionObj.questionType = 'scale';
            questionObj.properties.minLimit = lowerLimit;
            questionObj.properties.maxLimit = upperLimit;
            questionObj.properties.stepSize = 1;
            formObj.questions.push(questionObj);
          ...
          if(questionType === 'Group' || groupedQuestionChild) {
            ...
            if(groupType && groupType.toLowerCase() == 'no repeat'){
              ...
              formObj.questions.push(questionObj);
              parentId = questionObj._id;
            } else if(groupType && (groupType.toLowerCase() == 'selected' || groupType.toLowerCase() == 'notselected')){
              ...
              questionObj.groupQuestions.parantSettings.groupLabels = groupLabels.split(',');
              formObj.questions.push(questionObj);
              parentId = questionObj._id;
            } else {
      ...
      if(formObj.questions.length){
            ...
            var groupChildArr = childQuestionsArr.reduce(function(results, data) {
              (results[data.parentId] = results[data.parentId] || []).push(data._id);
              return results;
            }, {});
            
            for(let i = 0; i < formObj.questions.length; i++){
              if(formObj.questions[i]._id in groupChildArr) {
                formObj.questions[i].groupQuestions.childQuestions = groupChildArr[formObj.questions[i]._id]
              }
              for(let id in groupDependentsOn){
                  if(id == formObj.questions[i].importId){
                    parantQuestionId[groupDependentsOn[id]] =  formObj.questions[i]._id
                    break;
                  }   
              }
            ...
            for(let i = 0; i < formObj.questions.length; i++){
              for(let id in parantQuestionId) {
                if(id == formObj.questions[i]._id) {
                   formObj.questions[i].groupQuestions.parantSettings.questionId = parantQuestionId[id]
                   break;
                }   
              }
            }
            ...
            let skippedQuestions = Object.keys(groupedArr).length - formObj.questions.length;
        ...
        return Promise.resolve({
          message: "The uploaded file doesn't have any questions. Please check the type and format of the csv file.",
          status:0,
        });
      }
  ...
  async importGroupRes(file, mainBody) {
    try {
              ...
              $filter: {
                ...
                cond: {
                  $and: [
                    ...
                    { $in: ["$$questions._id", getGrpData.questions[0].groupQuestions.childQuestions] },
                  ],
                },
              },
            },
      ...
      for (let row of jsonData) {
        const getMainResData = await Response.findOne({importedId: row["Response ID"]});
        if (!getMainResData) {
          const err = new Error(error_code.UNKNOWN_ERROR.CODE);
          err.statusCode = 0;
          throw err;
        ...
        const keyObj = Object.keys(row).slice(Object.keys(row).indexOf("Group Context")+1, Object.keys(row).length);
        const responses = []; let idx=0;
        for (let formQues of allQuestions[0].questions) {
          ...
          if (Object.keys(resValueObj).length !== 0) {
            const resObj = {
              ...
              isParent: true,
              groupId: grpQuesId,
              groupLabelId: row["Group Context"] ? row["Group Context"] : undefined
            };
            // responses.push(resObj);
            const updateRes = await Response.updateOne({importedId: row["Response ID"]}, {
  ...
  timeToTimestamp(time) {
    ...
    if (!s) {s=h, h=undefined}
    h=parseInt(h);m=parseInt(m);s=parseInt(s);
    if (h) { h=parseInt(h); h <= 0 ? h=1 : h=h*60*60*1000; }
    m <= 0 ? m=1 : m=m*60*1000;
    s <= 0 ? s=1 : s=s*1000;
    return (h ? h : 0)+m+s;
  }

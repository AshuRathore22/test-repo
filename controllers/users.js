...
exports.setUserPassword = async (req, res, next) => {
  try {
    ...
    //     'From': '+15673721184',
    //     'To': '+919479555899',
    //     'Body': 'OTP for Accrue is: ' + OTPMobile
    //   }
    // };
...
exports.deletePermanentUser  = async (req, res, next) => {
 try {
  const checkValidUser = await User.findById(req.sessionUserData.userid);
  if (!checkValidUser) {
    const err = new Error(error_code.USER_NOT_FOUND.CODE);
  ...
  const deletedUser = await User.findOneAndDelete({ _id: req.sessionUserData.userid });
  if (!deletedUser) {
    const err = new Error(error_code.USER_NOT_FOUND.CODE);
    err.statusCode = 0;
    throw err;
